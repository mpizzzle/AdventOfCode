...
(define (count-trees tree-map x y slope)
  (if (not (null? tree-map))
    (let ((parity (= (modulo y (car (cdr slope))) 0)))
      (+ (if parity
        (+ (check-slice (car tree-map) x)) 0)
        (count-trees (cdr tree-map) (+ x (if parity (car slope) 0)) (+ y 1) slope)))
    0))
...
(display (count-trees input 0 0 (car (cdr slopes)))) (newline)
(display (apply * (map (lambda (slope) (count-trees input 0 0 slope)) slopes))) (newline)
